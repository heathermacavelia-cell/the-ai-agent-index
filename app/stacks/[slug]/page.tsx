...
export default async function StackPage({ params }: { params: { slug: string } }) {
  ...
  const { data: agents } = await supabase
    .from('agents')
    .select('slug, name, short_description, website_url, favicon_domain, logo_url, mcp_compatible, mcp_status, starting_price, pricing_model, is_affiliate, affiliate_url, last_verified_at, vendor_tracking_url')
    .in('slug', agentSlugs)
          ...
          {steps.map((step, i) => (
            <div key={step.id}>
              <div style={{ backgroundColor: 'white', border: '1px solid #E5E7EB', borderRadius: '0.875rem', padding: '1.5rem', display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: '1rem', alignItems: 'start' }}>
                ...
                <a href={visitHref(step.agent)} target="_blank" rel={outboundRel({ affiliate: !!step.agent.affiliate_url, verified: !!step.agent.last_verified_at })} data-out={step.agent.slug} data-out-from="stack"
