...
export async function GET() {
  ...
  // Get 30 most recently added agents
  ...
  // Get 20 most recently updated agents (where updated_at > created_at)
  ...
  const changelog = {
    ...
    recent_updates: (recentUpdates ?? [])
      .filter((a) => {
        ...
        return updated - created > 60000; // only show if updated >1min after creation
      })
      .map((a) => ({
        name: a.name,
        slug: a.slug,
