...
const steps = [
  {
    number: '01',
    title: 'Define what the agent replaces or augments',
  },
  {
    number: '02',
    title: 'Calculate the value of time saved',
  },
  {
    number: '03',
  ...
  {
    number: '05',
    title: 'Apply the formula and interpret the result',
  },
]
...
const faqItems = [
  {
    ...
    a: 'ROI equals the sum of revenue generated and the monetary value of time saved, minus the total tool cost, divided by the total tool cost, multiplied by 100. In formula form: ROI = ((Revenue Generated + Time Value Saved) - Total Tool Cost) / Total Tool Cost x 100. A result above 0 percent means the agent is generating more value than it costs. The most common mistake is calculating time saved without monetizing it. Hours recovered only become ROI-relevant when valued at the effective hourly cost of the person whose time was saved.',
  },
  {
...
export default function AIAgentROIPage() {
  ...
  const itemListLd = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: 'AI Agent ROI Calculation Steps',
    ...
    <div style={{ maxWidth: '780px', margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
      ...
      <div style={{ backgroundColor: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '3rem' }}>
        ...
        <p style={{ fontSize: '0.9375rem', color: '#0C4A6E', lineHeight: 1.6, margin: 0 }}>
          ROI = ((Revenue Generated + Time Value Saved) - Total Tool Cost) / Total Tool Cost x 100
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.875rem', marginBottom: '1.25rem' }}>
        {[
          ...
          { label: 'Increases output without headcount', desc: 'The same team produces more: more outreach sent, more tickets resolved, more content published. ROI comes from the output multiplier, which is the value of additional output minus the tool cost.' },
          { label: 'Improves conversion or quality', desc: 'The agent makes existing activity more effective: better personalization, faster response times, fewer dropped leads. ROI comes from revenue lift or cost reduction per unit of output.' },
        ].map((item) => (
      ...
      <p style={{ fontSize: '0.9375rem', color: '#374151', lineHeight: 1.75, marginBottom: '1.25rem' }}>
        The subscription price is only one component of total cost. The costs most commonly omitted are management time (the weekly hours your team spends configuring, reviewing, and maintaining the agent) and integration costs, which are a one-time investment that should be amortized across the contract period. Both can be significant enough to change the ROI calculation materially.
      </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: '0.875rem' }}>
          ...
          <tbody>
            {[
              ...
              ['Per-seat fees', 'Active seats x per-seat monthly cost', 'Always'],
              ['Setup and integration', 'Total one-time cost / 12 months', 'Always'],
              ['Ongoing management time', 'Hours/month x team hourly rate', 'Always'],
              ['Training and onboarding time', 'Total onboarding hours x hourly rate / 12', 'First year only'],
      ...
      <div style={{ marginBottom: '2rem' }}>
        <NewsletterSignup sourcePage="guide-how-to-calculate-ai-agent-roi" sourceType="guide" />
      </div>
