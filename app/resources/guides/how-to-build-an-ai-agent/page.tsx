...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'How to Build an AI Agent (2026): Code + Cost',
    description: 'TypeScript code, build vs buy framework, cost math at 1K tickets/day, and 5 production failure modes with fixes. Pre-deploy checklist included.',
  ...
  twitter: {
    card: 'summary',
    title: 'How to Build an AI Agent (2026): Code + Cost',
    description: 'TypeScript code, build vs buy, cost math, and 5 failure modes. Practical guide for builders.',
  },
  alternates: {
...
export default async function HowToBuildAnAIAgentPage() {
  const meta = await getGuideMeta('how-to-build-an-ai-agent')
  ...
  const jsonLd = {
    ...
    '@type': 'Article',
    headline: 'How to Build an AI Agent (2026): Code + Cost',
    description: 'TypeScript code, build vs buy framework, cost math at 1K tickets/day, and 5 production failure modes with fixes.',
    url: 'https://theaiagentindex.com/resources/guides/how-to-build-an-ai-agent',
  ...
  const itemListLd = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: 'AI Agent Production Failure Modes',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Hallucinated tool calls' },
      { '@type': 'ListItem', position: 2, name: 'Infinite loops' },
      { '@type': 'ListItem', position: 3, name: 'Parameter type errors' },
      { '@type': 'ListItem', position: 4, name: 'Context window blow-out' },
      { '@type': 'ListItem', position: 5, name: 'Cost runaway' },
    ],
  }
  ...
  return (
    <div style={{ maxWidth: '780px', margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
      ...
      <p style={{ fontSize: '0.9375rem', color: '#4B5563', lineHeight: 1.75, marginBottom: '1.25rem' }}>
        From Anthropic&apos;s research: workflows are systems where LLMs and tools are orchestrated through predefined code paths. Agents are systems where LLMs dynamically direct their own processes and tool usage. The implication is significant. If your problem can be solved with a fixed sequence of LLM calls and known tools, you do not need an agent. You need a workflow, which is dramatically simpler to build and maintain.
      </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
          ...
          <tbody>
            {[
              ...
              ['Claude Sonnet 5', '$2 / $10', 'Near-Opus reasoning at Sonnet pricing. Introductory rate through Aug 31, 2026.'],
              ['Claude Sonnet 4.6', '$3 / $15', 'Proven stable model. Complex reasoning, long-context tasks.'],
              ['GPT-4.1', '$2 / $8', 'OpenAI recommended production model. 1M context window. No vision.'],
              ['Gemini 2.5 Pro', '$1.25 / $10', 'Massive context windows (1M+), multimodal, strong coding.'],
  ...
  throw new Error("Agent exceeded max iterations — possible loop")
      ...
      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.875rem', marginBottom: '2rem' }}>
        {[
          ...
          },
          {
            ...
            fix: 'Summarize old messages once history exceeds a threshold (e.g. > 50K tokens). Replace the oldest 20 messages with a 200-token summary. Lose detail, keep coherence.',
          },
          {
            name: '5. Cost runaway',
      ...
      <p style={{ fontSize: '0.9375rem', color: '#4B5563', lineHeight: 1.75, marginBottom: '1rem' }}>
        Here&apos;s a real calculation for our customer support triage agent at production scale. Assumptions: 1,000 tickets per day, average 3 tool calls per ticket, roughly 4,000 input tokens (system prompt + tool definitions + conversation context) and roughly 500 output tokens per LLM call.
      </p>
      <div style={{ backgroundColor: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '1.25rem' }}>
        <p style={{ fontSize: '0.8125rem', fontFamily: 'monospace', color: '#374151', lineHeight: 1.7, margin: 0 }}>
          Per ticket: 3 LLM calls x (4,000 input + 500 output tokens)<br/>
          ...
          <br/>
          <strong style={{ color: '#111827' }}>Claude Haiku 4.5:</strong> (360 x $1) + (45 x $5) = <strong>$585/mo</strong><br/>
          <strong style={{ color: '#111827' }}>Claude Sonnet 5:</strong> (360 x $2) + (45 x $10) = <strong>$1,170/mo</strong><br/>
          <strong style={{ color: '#111827' }}>GPT-4.1:</strong> (360 x $2) + (45 x $8) = <strong>$1,080/mo</strong><br/>
          <strong style={{ color: '#111827' }}>Claude Sonnet 4.6:</strong> (360 x $3) + (45 x $15) = <strong>$1,755/mo</strong>
        </p>
      </div>
      <p style={{ fontSize: '0.875rem', color: '#6B7280', lineHeight: 1.7, marginBottom: '2rem' }}>
        Two things this calculation hides: prompt caching can cut input costs 70-90% if you reuse a stable system prompt, and context growth on long conversations can double these numbers if you don&apos;t implement summarization. Budget conservatively. Your real cost will be 1.5-2x your napkin math the first time.
      </p>
      ...
      <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column' as const, gap: '0.5rem', marginBottom: '2rem' }}>
        {[
          ...
          'Context summarization kicks in before hitting model limits.',
          "You've manually traced 10 representative runs end-to-end.",
          'Rollback plan exists if production behavior diverges from eval.',
        ].map((item, i) => (
      ...
      <p style={{ fontSize: '0.875rem', color: '#9CA3AF', lineHeight: 1.6 }}>
        All agents listed are editorially reviewed by The AI Agent Index. See our <Link href="/methodology" style={{ color: '#6B7280' }}>editorial methodology</Link>. Pricing data verified July 2026. Confirm current rates before budgeting.
      </p>

      <div style={{ marginBottom: '2rem' }}>
        <NewsletterSignup sourcePage="guide-how-to-build-an-ai-agent" sourceType="guide" />
      </div>
