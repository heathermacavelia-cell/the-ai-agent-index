  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <span className={`flex-shrink-0 text-xs font-medium px-2 py-1 rounded-md ${PRICING_COLORS[agent.pricing_model] ?? 'bg-gray-100 text-gray-600'}`}>
          {agent.pricing_model}
        </span>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1">{tagElements}</div>
      {meta && (
        <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-1.5">
          <span className="text-xs">{meta.icon}</span>
          <span className={`text-[11px] font-medium ${meta.color}`}>
            {agent.primary_category.replace('ai-', '').replace(/-/g, ' ')}
          </span>
        </div>
      )}
    </Link>
...
export default async function HomePage() {
  ...
  const categoryCards = []
  for (const [displayName, slug] of categoryEntries) {
    ...
    categoryCards.push(
      <Link
        key={slug}
        href={`/${slug}`}
        className={`group block bg-white rounded-xl border ${meta?.borderColor ?? 'border-gray-200'} p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5`}
      >
  ...
  for (const agent of featuredAgents) {
    agentCards.push(<AgentCard key={agent.id} agent={agent} />)
  }

  const statItems = [
    { label: 'Agents indexed', value: String(totalAgents) },
    { label: 'Categories', value: '5' },
    { label: 'Schema fields', value: '30+' },
    { label: 'JSON API endpoints', value: '1' },
  ]
  const statElements = []
  ...
  const apiTags = ['JSON-LD schema', 'Public JSON API', 'Structured taxonomy', 'Clean URLs', 'Dynamic sitemap']
  const apiTagElements = []
  for (const tag of apiTags) {
    apiTagElements.push(
      <span key={tag} className="px-2.5 py-1 rounded-md bg-gray-900 border border-gray-700 text-gray-400 text-xs font-mono">
        {tag}
      </span>
    )
  }
    ...
    <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-900 border border-gray-700 text-gray-400 text-xs font-mono mb-6">
              ...
              {totalAgents} agents indexed · public JSON API
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                ...
                className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg border border-gray-700 hover:border-gray-500 text-gray-400 hover:text-white font-mono text-sm transition-colors"
              >
                GET /api/agents
              </a>
            </div>
      ...
      <section id="categories" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="text-xs font-semibold text-blue-600 uppercase tracking-widest mb-1.5">Browse by function</p>
      ...
      {featuredAgents.length > 0 && (
        <section className="border-t border-gray-100 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div className="flex items-end justify-between mb-8">
              <div>
                <p className="text-xs font-semibold text-blue-600 uppercase tracking-widest mb-1.5">Editorially selected</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 font-mono text-sm overflow-x-auto">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-2 text-gray-600 text-xs">GET /api/agents?category=ai-coding-agents</span>
              </div>
