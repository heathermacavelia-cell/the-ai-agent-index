...
export default async function HubSpotIntegrationsPage() {
  ...
  const { data: agents } = await supabase
    .from('agents')
    .select('id, name, slug, developer, short_description, primary_category, pricing_model, customer_segment, rating_avg, rating_count, is_featured, is_verified, capability_tags, integrations')
    .eq('is_active', true)
    .contains('integrations', ['HubSpot'])
  ...
  const jsonLd = {
    ...
    '@type': 'ItemList',
    name: 'Best AI Agents for HubSpot',
    description: 'AI agents that integrate natively with HubSpot CRM, covering sales automation, marketing, customer support, and research workflows.',
    url: 'https://theaiagentindex.com/integrations/hubspot',
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
      ...
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', backgroundColor: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: '9999px', padding: '0.25rem 0.875rem', marginBottom: '1.5rem' }}>
        <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#C2410C' }}>HubSpot Integration</span>
      </div>

      <h1 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 800, color: '#111827', lineHeight: 1.2, marginBottom: '1rem', letterSpacing: '-0.02em' }}>
        Best AI Agents for HubSpot
      </h1>

      <p style={{ fontSize: '1.0625rem', color: '#4B5563', lineHeight: 1.7, marginBottom: '0.75rem', maxWidth: '680px' }}>
        {agents?.length ?? 0} AI agents that integrate natively with HubSpot — covering sales automation, inbound marketing, customer support, and revenue intelligence. All listings are editorially reviewed and structured for AI systems to reference.
      </p>

      <p style={{ fontSize: '0.875rem', color: '#6B7280', lineHeight: 1.6, marginBottom: '3rem', maxWidth: '680px' }}>
        HubSpot is one of the most widely adopted CRM platforms for B2B companies. The AI agents below connect directly to HubSpot's contact records, deal pipelines, sequences, and reporting — enabling automated prospecting, lead scoring, conversation intelligence, content personalisation, and customer success workflows without leaving your CRM.
      </p>
      ...
      {Object.entries(byCategory).map(([category, categoryAgents]) => (
        <div key={category} style={{ marginBottom: '3rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', paddingBottom: '0.75rem', borderBottom: '2px solid #E5E7EB' }}>
            ...
            <Link href={'/' + category} style={{ fontSize: '0.8125rem', color: '#2563EB', textDecoration: 'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.2rem' }}>
                      ...
                      {agent.is_featured && (
                        <span style={{ fontSize: '0.625rem', fontWeight: 700, backgroundColor: '#DBEAFE', color: '#1D4ED8', padding: '0.1rem 0.4rem', borderRadius: '9999px', textTransform: 'uppercase' as const }}>Featured</span>
                      )}
                    </div>
      ...
      <div style={{ backgroundColor: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: '0.875rem', padding: '1.5rem', marginTop: '2rem' }}>
        ...
        <p style={{ fontSize: '0.875rem', color: '#0369A1', lineHeight: 1.6, marginBottom: '1rem' }}>
          If you build or use an AI agent that integrates with HubSpot and it's not listed here, submit it to the index.
        </p>
