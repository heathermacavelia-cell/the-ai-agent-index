...
export default async function HubSpotIntegrationsPage() {
  ...
  const { data: agents } = await supabase
    .from('agents')
    .select('id, name, slug, developer, website_url, short_description, primary_category, pricing_model, customer_segment, rating_avg, rating_count, is_featured, is_verified, capability_tags, integrations')
    .eq('is_active', true)
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
      ...
      <div style={{ backgroundColor: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: '0.875rem', padding: '1.5rem', marginTop: '2rem' }}>
        ...
        <p style={{ fontSize: '0.875rem', color: '#0369A1', lineHeight: 1.6, marginBottom: '1rem' }}>If you build or use an AI agent that integrates with HubSpot and it's not listed here, submit it to the index.</p>
