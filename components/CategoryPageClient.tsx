...
export default function CategoryPageClient({ agents }: { agents: Agent[] }) {
  ...
  const sorted = useMemo(() => {
    ...
    if (sort === 'price_asc') {
      return list.sort((a, b) => (a.starting_price ?? 0) - (b.starting_price ?? 0))
    }
    if (sort === 'price_desc') {
      return list.sort((a, b) => (b.starting_price ?? 0) - (a.starting_price ?? 0))
    }
    return list
  }, [agents, sort])
          ...
          <div style={{ display: 'flex', gap: '0.375rem' }}>
            {([
              ...
              <button key={opt.value} onClick={() => setSort(opt.value)}
                style={{ padding: '0.375rem 0.75rem', borderRadius: '0.375rem', border: '1px solid', fontSize: '0.8125rem', fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s', backgroundColor: sort === opt.value ? '#2563EB' : 'white', color: sort === opt.value ? 'white' : '#374151', borderColor: sort === opt.value ? '#2563EB' : '#E5E7EB' }}>
                {opt.label}
              </button>
            ))}
          </div>
