...
export default function CompareBar() {
  ...
  const boardUrl = '/compare/build?agents=' + agents.map(a => a.slug).join(',')
  ...
  return (
    <>
      ...
      >
        <span style={{ position: 'absolute', top: '-4px', right: '-4px', backgroundColor: '#111827', borderRadius: '50%', width: '22px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6875rem', fontWeight: 700, border: '2px solid white' }}>
          {count}
        </span>
      ...
      {expanded && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem' }}>
            <button
              onClick={() => { setExpanded(false); router.push(boardUrl) }}
      ...
      <style>{`
        @media (max-width: 768px) {
          .compare-bar-desktop { display: none !important; }
        }
        @media (min-width: 769px) {
          .compare-bar-mobile { display: none !important; }
        }
      `}</style>
    </>
  )
